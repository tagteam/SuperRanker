...
cross-validation procedure. The tuning parameter for Lasso and Ridge regression
...
600. At deeper list depths all agreement curves become approximately constant.
...
\textbf{CHECK UP: CENSOR ON POOLED DATA OR CENSOR BY INDIVIDUAL THRESHOLD}



\textbf{DET HER ER NOGET VÆRRE ROD. DER ER FORSVUNDET NOGET LASSO TEKST. GÅ TILBAGE OG FIND DET IGEN}
Under the deviance optimization criterion the median number of non-zero
coefficients was 33 (range 18 to 52). In order to calculate the sequential
rank agreement for the Lasso we therefore applied our proposed Algorithm \ref{sra-algorithm}
...
\textbf{ANDREAS: man kan desvaerre ikke se hvor Lasso agreement forsvinder
...
\textbf{ANDREAS og CLAUS: er det ogsaa forvented eller skyldes det at vi kumulerer fra toppen? agreement boer jo igen vaere hoejt bland low risk patienter ... Claus indsætter måske noget her!}
...
\textbf{Andreas: jeg forstaar ikke den sidste saetning. kan man sammenligne de to rank agreements (left vs right panel)?}
...
\textbf{and for this data set it appears that PLS-DA is the overall best
