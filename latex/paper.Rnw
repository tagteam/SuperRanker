...
Each spectrum consists of $49642$ samples over a mass-to-charge range
of $800$ to $20000$ which we down-sample on an equidistant grid of
5000 points by linear interpolation. After this dimension reduction
the mass spectrum is represented by 5000 predictors. We preprocess
%% ANDREAS: det er uklar om preprocessing forgaar i de oprindelig spectra eller i de down-samplede 
each spectrum by first removing the slow-varying baseline intensity
with the SNIP algorithm \citep{ryan1988snip} followed by a
normalization with respect to the total ion count. We then standardize
...
latter is important, e.g., to rank patients according to immediacy of
treatment.
...
For each of the four methods, each of the 1000 models trained in on of
...
All four methods depend on tuning parameters. We select these
separately for each sub-sample by a 20 times repeated 5-fold
cross-validation.
%% ANDREAS: hvilke tuning parameters har du optimeret for Random forest og PLS-DA?
...
the value of the penalty parameter. Under the deviance optimization
criterion the median number of non-zero coefficients was 33 (range 18
to 52). In order to calculate the sequential rank agreement for the
LASSO we therefore applied our proposed Algorithm \ref{sra-algorithm}
with $B = 1000$ and $P=5000$.

For the Random Forest classifier the predictors are ranked according
to the Gini index, while for the logistic LASSO and Ridge regression
models we order the predictors by absolute magnitude of the estimated
...
120. At deeper list depths all agreement curves appeared stable.
...
\end{center}
%% ANDREAS: kan du aendre titel for begge grafer:
%% "Risk probabilities" -> "Predicted risks of malignant tumor in 113 patients (DACOVA)"
%% "Predictor importance" -> "Discrimination importance of 5000 predictors"
\caption{Left panel: Sequential rank agreement of 1000 rankings of the
  ...
  malignant tumor in the 113 MALOVA patients. Right panel: Sequential
...
remaining items become close to random.
%% ANDREAS: boer "close to random" ikke vaere ens for alle metoder?
...
distribution of the regression coefficients we see that a large
...
alleviated by artificially censoring all predictors with absolute
coefficient values below a given threshold. For the Rigde models tuned
with the deviance criterion, Figure \ref{fig:app2} (left) shows the
the sequential rank agreement where for each of the 1000 trained
models predictors were artificially censored when their absolute
coefficient value was lower than the $0.1\%$ quantile of the 5000
absolute coefficient values. The corresponding curve from Figure
\ref{fig:app1} (left panel) is shown for comparison.
%% ANDREAS: godkender du min aendring af de sidste 2 saetninger?
Thus, in our application the median number of censored predictors
across the 1000 sub-samples was 5 (range 1 to 11). Even though the
number of censored predictors is very small compared to the total
...
is as expected, starting low and then increasing. Also, the effect of
the censoring caused by the sparsity of the LASSO is clearly visible
in the agreement curves.
%% ANDREAS: man kan desvaerre ikke se hvor LASSO agreement forsvinder
%% (ved list-depth 5?) og om den er laver for list depth 1-5 end rigde
%% eller om den er kun laver for list depth 1-2. maaske kan du
%% beskrive dette i teksten?
...
predicted probabilities are then ranked in decreasing order such the
patients with the highest risk of a malignant tumor appears on the top
of the list. Figure \ref{fig:app1} (left) shows the sequential rank
agreement separately for each method, based on 1000 risk predictions
obtained from the models trained in the same $1000$ random sub-samples
of the MALOVA data. For clarity of presentation we zoom in on the
agreement up to list depth 600. At deeper list depths all agreement
curves appeared stable.

Most curves start low and then increase indicating higher agreement
among high risk patients.
%% ANDREAS og CLAUS: er det ogsaa forvented eller skyldes det at vi kumulerer fra toppen?
%% agreement boer jo igen vaere hoejt bland low risk patienter ...
An exception is the risk prediction agreement for the LASSO tuned with
the AUC criterion which shows very low agreement among the high values
of the predicted risks. The reason is that optimizing the penalty
parameter with respect to the AUC criterion tends to favor a very high
penalty value causing only a single predictor to be selected in each
of the 1000 iterations. This results in a lack of generalizability to
the DACOVA data which gives rise to the higher disagreement in the
predicted risk probabilities. In the extreme case where the penalty
becomes so high that none of the predictors are selected by the LASSO,
the sequential rank agreement for the predicted probabilities becomes
undefined since all the ranks will be ties.
...
In Figure \ref{fig:app1} it can further be seen that some of the
methods show better agreement with respect to the predicted
probabilities than for ranking the importance of the predictors and
vice versa. Ridge regression shows higher agreement across training
sets for the risk predictions than PLS-DA, and PLS-DA shows higher
agreement for predictor importance than Ridge regression. The Random
Forest method seems to perform equally well in both scenarios.
%% Andreas: jeg forstaar ikke den sidste saetning. kan man sammenligne de to rank agreements (left vs right panel)?
LASSO shows similar agreement for ranking the risk predictions than
PLS-DA (unless with the AUC criterion), and poor agreement for ranking
...
which specific variable is selected from a group of highly correlated
...
predictions. Thus, we also computed the AUC for the different models
...
AUC values with a median value of $0.71$ while the Ridge model with
...
%% and for this data set it appears that PLS-DA is the overall best
%% method taking both agreement and discrimination ability into account.
%% Andreas: vi boer ogsaa se paa Brier score (calibration) inden vi konkluderer at PLS-DA er den bedste
...
%% Claus: hvad vil du sige med den sidste saetning?
...
agreement among the items that were deemed to relevant. In our
...
%% applied to a large number of situations.
%% CLAUS: den sidste saetning er bar en omformulering af Diskussionen's foerste saetning.

%% The method here proposed can be adapted further by using it to
%% identify and cluster statistical analysis methods that provide more or
%% less the same agreement,
%% Claus: det er enten for kort eller for lang


%% or by using the rank agreement to optimize a
%% hyper-parameter in, say, elastic net regularized regression where the
%% rank agreement is used to determine the mixing proportion between the
%% $L_1$ and the $L_2$ penalty.
%% Claus: det er enten for kort eller for lang

%% Finally, the proposed method may be
%% adapted (with some additional assumptions) to the situation where the
%% lists are subsets of each other.
%% Claus: det er enten for kort eller for lang

%% We will be investigating these
%% extensions further in the future.
