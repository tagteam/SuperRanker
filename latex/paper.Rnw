...
For the fully obseved list situation we can plot the sequential rank
agreement \eqref{def:sra} as a function of depth $d$.  An example is
seen in Figure~\ref{fig:example1} where smaller values on the $y$ axis
correspond to better agreement among the lists than larger values. The
sequential rank agreement at depth $d$ can be interpreted as how many
ranks apart do we on average expect the items found until depth $d$ to
be.
...
in the rank agreement levels then this suggests that there are changes
in the levels of agreement. In particular, we expect the agreement
among the lists to be better towards the top of the lists and worse
towards the end of the lists.
...
example, comparison of top $k$ lists, when some methods only rank some
of the items (e.g., penalized regression based on the Lasso provides a
sparse set of predictors that have non-zero coefficients. There is no
obvious ordering of the predictors that have been shrunk to zero), or
if 

Top $k$, methods only provide information on some ranking (Lasso),
only interested in significant ...predictors 
...
inputmatrix2[51:nrow(inputmatrix),] <- 0
...
<<fig2a, fig=TRUE,echo=FALSE,include=FALSE>>=
ysize <- 1000
plot(res[1:ysize], lwd=3, col="red", type="l", ylim=c(0,1300), ylab="Sequential rank agreement", xlab="Depth")
lines(res2[1:ysize], lwd=2, col="purple")
## 
...
<<fig2b, fig=TRUE,echo=FALSE,include=FALSE >>=
ysize <- 80
plot(res[1:ysize], lwd=3, col="red", type="l", ylim=c(0,1300), ylab="Sequential rank agreement", xlab="Depth")
lines(res2[1:ysize], lwd=2, col="purple")
## 
