...
All four methods depend on a tuning parameter. We select these
separately for each sub-sample by a 20 times repeated 5-fold
cross-validation procedure. For Lasso and Ridge regression the tuning parameter
is the degree of penalization, for PLS-DA it is the number of components 
(the dimensionality of the subspace), and for Random Forest it is the number
 of predictors randomly sampled at each split.
...
coefficient values below a given threshold. For the Ridge models tuned
