...
hypotheses. We wish to determine if we observe better agreement that
...
\begin{eqnarray*}
H_1 & :&  \text{The list rankings correspond to ranked lists based on}\\
& &   \text{data containing no information}
\end{eqnarray*}
...
Compute the sequential rank agreement for a large number of random
permutations ... with the same number of lists and with the same amount of censoring.


understood about the outcome

Clearly hypothesis .... is ... no association and that the methods are
also completely independent.

Our data free benchmark value is
...
\end{center}
 \caption{Average overlap (left figure) and sequential rank agreement
   (right figure) for two different analysis methods
