...
\citep{Reshef2011} which resulted in the rankings seen in
...
\end{center}
 \caption{Left panels: Sequential rank agreement for 4 different analysis methods
   ...
   Corresponding xssequential rank agreement for the same data but
...
The proposed approach for censored rankings is based on two
assumptions: 1) that the items of interest will be ..., and that 2)
that the censored rankings provide so little information that it is
reasonable to assume that the missing rankings 


For censored data it is also imminent that ...



Discuss assumption

Present stuff that is long enough

information 


For censored lists we expect a change point around the depth
