...
\end{equation}
Here $|A|$ is the cardinality of the set $A$.  Typically the overlap
is only considered for pairwise comparisons but formula
\eqref{def:overlap} can be applied even when $L>2$
...
distributions and simple point-wide $p$-values can be obtained for
...
from the lists as possible, i.e., we have used all available ranks. If
we have information on the rankings as well as the corresponding
evidence for significance that we could choose to censor the rankings
of all the items that were deemed not to be significant. That would
...
rank agreement as a hyper-parameter in, say, elastic net regularized
