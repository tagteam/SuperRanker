...
To obtain the distribution under $\widetilde H_0$ the idea is to repeat
...
$\wildetilde H_0$ each based on $400$ permutations. Not surprisingly, the
sequential rank agreement under $\widetilde H_0$ is lower than the
...
\textbf{Our applications show that XXX
