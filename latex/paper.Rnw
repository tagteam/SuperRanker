...
subsample of $119$ patients with a total of $58$ patients with
...
1990. Similarly, we use a random subsample from the DACOVA study of
...
agreements are assessed with respect to
perturbations of the training data in the following manner. We
repeatedly draw a sub-sample (without replication) consisting of 90\%
of the MALOVA observations and train the four models on the
sub-sample. Each of the models produces a ranking of the 5000
predictors according to their importance with respect to
discriminating between the tumor types, and the agreement of these
rankings across the different sub-samples are used to assess the
stability of ranking the importance of the predictors. For the Random
Forest classifier the predictors are ranked according to the Gini
index, while for the logistic Lasso and Ridge regression models we
order the predictors by absolute magnitude of the estimated regression
coefficients. For the PLS-DA model the importance of the predictors is
based on a weighted sum of the absolute coefficients where the weights
are proportional to the reduction in the sums of squares across the
components. To assess the stability of the individual risk predictions
we apply the predictors from the DACOVA data set to each of the
models. These predicted probabilities are then ranked in decreasing
order, and the agreement of the risk predictions are similarly
obtained by the sequential rank agreement applied across the different
sub-samples of the training data. We use 1000 iterations for the
sub-sampling procedure.

All four models depend on tuning parameters. We estimate these by a 20
times repeated 5-fold cross-validation on the training data in each
iteration. The implementation of Lasso and Ridge regression in the
...
criteria to our data to investigate their effect on the agreement.
...
Figure \ref{fig:app1} (left) shows the sequential rank agreement
applied to the estimated risk probabilities of the $113$ patients in
the DACOVA data set obtained from the $1000$ sub-samples of the MALOVA
data, and Figure \ref{fig:app1} (right) shows the sequential rank
agreement of the estimated importance of the 5000 predictors. For
clarity we only show the agreement for the latter up to list depth 600
where all the curves have approximately become stable.  The Lasso
models produce censored lists depending on the value of the penalty
parameter. Under the deviance optimization criterion the median number
of non-zero coefficients was 33 (range 18 to 52). In order to
calculate the sequential rank agreement we therefore applied Algorithm
\ref{sra-algorithm} with $B = 1000$ and $P=5000$. The effect of the
censoring caused by the sparsity of the Lasso is clearly visible in
the agreement curves.
...
alternative measures such as the mean absolute deviance may prove more
