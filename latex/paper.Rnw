...
predictors because we only observe a censored version of $\tilde R$
...
have \emph{not} been assigned for list $l$,
\begin{equation}
\tilde A(X_p) = \frac{\sum_{r_1; r_1\in \tilde R_1(X_p)}  \cdots \sum_{r_L; r_L\in \tilde R_L(X_p)} A(X_p)}{\prod_l |\tilde R_l(X_p)|}.
\end{equation}

The rankings within a single list are clearly not independent since
each of the lists essentially contains full set of ranks and by
