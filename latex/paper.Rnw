...
\end{equation}
where $|S_d|$ refers to the cardinaly of the set $S_d$. Values of
...
\end{center}
 \caption{Sequential rank agreement of predicted risk probabilities for the DACOVA data (left) and estimated predictor importance (right) based on 1000 sub-samples of the MALOVA training data.}
 \label{fig:app1}
\end{figure}

Figure \ref{fig:app1} (left) shows the sequential rank agreement applied to the estimated risk probabilities of the $113$ patients in the DACOVA data set obtained from the $1000$ sub-samples of the MALOVA data, and Figure \ref{fig:app1} (right) shows the sequential rank agreement of the estimated importance of the 5000 predictors. For clarity we only show the agreement for the latter up to list depth 600 where all the curves have approximately become stable. We note here that the predictor importance estimated with the logistic Lasso regression model is capable of producing censored lists depending on the value of the penalty parameter. With the deviance criterion the median number of non-zero coefficients was 33 (range 18 to 52). In order to calculate the sequential rank agreement we therefore applied Algorithm \ref{sra-algorithm} with $B = 1000$ and $P=5000$. The effect of the censoring caused by the sparsity of the Lasso is clearly visible in the agreement curve for the importance of the predictors. 


For most of the sequential rank agreement curves we see as expected that they start low followed by an increase until they approximately become constant.  This has the interpretation that the classification methods show higher agreement with respect to the highest ranked items (the most high-risk patients and the most important predictors respectively) then for the items deeper in the lists across the different sub-samples. The change-point where the curves become approximately constant is the list depth where the ranks of the remaining items become almost random. The only exceptions are the predictor importance agreements using the logistic Ridge regression models and the risk probability agreement using the logistic Lasso regression with the AUC criterion which show higher disagreement in the top of the lists. The reason for this in the Lasso case is that the AUC criterion tends to favor a very high value of the penalty parameter causing only a single predictor to be selected in all of the 1000 iterations. This results in a lack of generalizability to the DACOVA data  giving rise to the higher disagreement in the predicted risk probabilities. In the extreme case where the penalty becomes so high that none of the predictors are selected by the Lasso, the sequential rank agreement for the predicted probabilities becomes undefined since all the ranks will be ties.

From Figure \ref{fig:app1} it can also be seen that some of the methods appear to be better at agreeing with respect to the predicted probabilities than at agreeing with respect to the ranking of the importance of the predictors and vice versa. Ridge regression performs very well for the risk predictions but is outperformed by PLS-DA for the predictor importances. On the other hand, PLS-DA appears to be similarly worse for the risk predictions compared to Ridge regression. The Random Forest method seems to perform equally well in both scenarios. Lasso regression (except with the AUC criterion as discussed previously) performs very well for the high-risk patients but very bad for the importance of the predictors. This reason for this is the high auto-correlation between the intensities in the mass spectra which leads to collinearity in the regression model. It is well-known that variable selection with the Lasso penalty does not preform very well when the predictors are highly correlated. The collinearity does, however, not affect the risk predictions, since it is not so important what specific variable that get selected from a group of highly correlated predictors when the purpose is risk predictions.

Finally, it would appear that Ridge regression with the AUC criterion for the penalty parameter achieves the best performance with respect to the stability of ranking the individual predicted risk probabilities. It must, however, be stressed that sequential rank agreement is only concerned with whether similar orderings of the patients is obtained under perturbations of the training data. It is not concerned with whether such an ordering is the correct one. This is similarly to the usual distinction between accuracy and precision. Looking at the distributions of the AUCs across the $1000$ iterations for each of the models with respect to the known class labels of the DACOVA data we see that the PLS-DA model attained the best AUC with a median value of $0.71$ (IQR = 0.07) while the Ridge regression with AUC criterion has a median AUC of $0.49$ (IQR = 0.02). This means that while Ridge regression with the AUC criterion achieves the highest rank agreement it performs similar to a random coin toss with respect to classification. Both concerns are of course important in practice and for this data set it appears that PLS-DA is the best method with respect to both concerns.
...
data as well as to evalaute the stability of the ranks from a single
...
whether they agree. The sequential rank agreemenet curve can be
...
indiviuals. Collinearity in particular can be a huge problem when
bootstrapping data or when comparing different analysis methods. For
example, marginal analyses where each item is analyzed seperately will
