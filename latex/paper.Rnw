...
patients that have low risk prediction.  An exception is the risk
prediction agreement for the Lasso tuned with the AUC criterion which
...
data which gives rise to the higher disagreement in the predicted risk
probabilities. In the extreme case where the penalty becomes so high
