...
\begin{abstract}
  ...
  identify change-points in the agreement of the lists. The
...
Key words: sequential rank agreement, partial ranking, order statistic, gene rankings,
...
potentially allows to infer a change-point, i.e., a list depth where
...
extensions. The approaches presented in this manuscript are available
in the R package \texttt{SuperRanker}.
...
$R(X_p)$ is the rank of item $X_p$. The inverse mapping $R^{-1}$
assigns to rank $r\in\{1,\dots,P\}$ the item $R^{-1}(r)$ that was
given rank $r$. An ordered list is the realization of a ranking
function $R$ applied to a data sample $X$ and we let
$R_1(X),\dots,R_L(X)$, $L\geq2$, denote ordered lists obtained from
$L$ different ranking functions applied to the same data $X$. We will
use the same notation for the equivalent scenario where we have
ordered lists obtained from applying the same ranking function to $L$
samples from the same population, i.e., for ranked lists
$R(X^1), \ldots, R(X^L)$.  Panels (a) and (b) of
Table~\ref{tab:example} show a schematic example of these
mappings. Thus if $R^{-1}(1)=X_{34}$ then item $X_{34}$ is ranked
...
other choices could be made. The sample standard error has an
interpretation as the average distance of the individual rankings of
the lists from the average ranking.
...
values suggests disagreement. If the ranked lists are identical then
the value of SRA will be zero for all depths $d$. The sequential rank
agreement can be interpreted as the average distance of the individual
rankings of the lists from the average ranking for each of the items
we have seen until depth $d$. Note that the terms that are part of
\eqref{def:sra} are not independent since each list contains the ranks
from 1 to $P$ exactly once. Hence we simply use the pooled standard
deviation as a measure of the average distance between rankings.
...
correlation which resulted in the top rankings seen in Table~\ref{tab1}.
...
that agreement is higher in the top of the lists (smaller values on
the $y$ axis) corresponding to \emph{better} agreement than in the
tails of the lists.
...
If the lists agree on all items then the sequential rank agreement is
 zero for all depths. Generally,  if there are changes in the level of rank
 agreement then this suggests that there are sets of items that are ranked
 similarly in all lists and other sets of items that have been assigned vastly
 different ranks in the lists. In gene association studies we
 generally expect the agreement among the lists to be better towards
 the top of the lists and worse towards the end of the lists. In this
 case the sequential rank agreement curve will start at a low level
 and then increase until it levels off as in
 Figure~\ref{fig:example1}.

\subsection{Analysis of incomplete/censored lists}

Incomplete lists are also a common occurrence. They arise for example
in case of missing data (items), when comparing top $d$ list results
from publications, or when some methods only rank a subset of the
items. For example, penalized regression based on the Lasso provides a
sparse set of predictors that have non-zero coefficients. There is no
obvious ordering of the set of predictors whose coefficient has been
shrunk to zero. Incomplete lists also occur when the analyst decides
to censor all lists at depth $d$ such that the measurement of
agreement is restricted to the first $d$ elements of each list.
...
\begin{equation}
  \tilde R_l(X_p) = \left\{\begin{array}{cl} \{R_l(X_p)\} & \text{ for } X_p\in \Lambda_l \\
...
result in less variation of the ranks and hence less variation of the
agreement, and it would artificially introduce a (downward) bias of
agreement for items that are censored in multiple lists.
...
H_0  & : &  \text{The list rankings correspond to complete randomly}\\
       &  & \text{permuted lists}.
\end{eqnarray*}
This does not only assume that there is no information in data on
...
For the analysis of the Golub data described in Section
\ref{sec:amfol}, Figure~\ref{fig:example1} shows the empirical
distributions of sequential rank agreement under $H_0$ and
$\tilde H_0$ each based on $400$ permutations. Not surprisingly, the
sequential rank agreement under $\tilde H_0$ is lower than the
...
generally tend to identify (and rank high) similar predictors even if
there are only spurious associations. The two bottom panels in
Figure~\ref{fig:example1} also indicate that the observed sequential
rank agreement (the black line) is better than what would be expected
by chance for data that contain no information since it lies below the
reference areas. However, the censored data also suggests that there
may be at most 1 or 2 ranked items towards the top of the lists that
yield a result better than what would be expected (the bottom-right
plot).
...
section we present an extension of the average overlap to more that
...
\end{equation}
Typically the overlap is only considered for pairwise comparisons but
formula \eqref{def:overlap} can be applied even when $L>2$
\citep{Bar-Ilan2006,Boulesteix2009}.
...
proposed average rank distance.
...
The black lines in Figure~\ref{fig:case1} show the average overlap and
...
that sequential rank is more robust against small perturbations of the
...
\section{Application}
We now consider an application of the sequential rank agreement to two data sets consisting of MALDI-TOF (Matrix-Assisted Laser Desorption/Ionization Time Of Flight) mass spectra obtained from blood samples from patients with either benign or malignant ovarian tumors. The data sets are subsamples of the Danish MALOVA and DACOVA study populations.

The MALOVA study is a multidisciplinary Danish study on ovarian cancer \citep{Hogdall:2004:Cancer:15160342} where all Danish women diagnosed with an ovarian tumor and referred for surgery from the participating departments of gynecology were enrolled continuously from December 1994 to May 1999. For the purpose of illustration we use a random subsample of $119$ patients with a total of $58$ patients with malignant ovarian cancers as cases and $61$ patients with benign
ovarian tumors as controls. The DACOVA study is another multidisciplinary Danish study on ovarian cancer which included about $66\%$ of the female population of Denmark \citep{bertelsen1991protocol}. It was aimed to continuously enroll all patients that were referred to surgery of an ovarian tumor clinically suspected to be cancer during the period 1984 to 1990. Similarly, we use a random subsample of $113$ patients with a total of $54$ malignant ovarian cancers and $59$ benign ovarian tumors/gynecologic disorders.

Each spectrum consists of $49642$ samples over a mass-to-charge range of $800$ to $20000$ which we down-sample on an equidistant grid of 5000 points by linear interpolation. We preprocess each spectrum by first removing the slow-varying baseline intensity with the SNIP algorithm \citep{ryan1988snip} followed by a normalization with respect to the total ion count. We then standardize the 5000 predictors to have column-wise zero mean and unit variance in each data set.

We use the two data sets to illustrate how the sequential rank agreement can be applied in two different scenarios. In the first scenario we assess the agreement of four different statistical classification methods in how they rank the predictors according to their importance with respect to the binary outcome. In the second scenario we assess the agreement among rankings of individual predicted risk probabilities of having a malignant tumor using the four methods. The first scenario is relevant in the context of biomarker discovery and the latter is important when e.g., ranking patients according to immediacy of treatment. 

We use the following four classification methods: Random Forest \citep{breiman2001random} implemented in the R package \texttt{randomForest} \citep{liaw2002classification}, logistic Lasso \citep{tibshirani1996regression} and Ridge regression \citep{segerstedt1992ordinary} both implemented in the R package \texttt{glmnet} \citep{friedman2010regularization}, and Partial Least Squares Discriminant Analysis (PLS-DA) \citep{boulesteix2004pls} implemented in the R package \texttt{caret} \citep{Jed-Wing:2014aa}.

In both scenarios we use the MALOVA data to train the statistical models, and the two types of agreements are assessed with respect to perturbations of the training data in the following manner. We repeatedly draw a sub-sample (without replication) consisting of 90\% of the MALOVA observations and train the four models on the sub-sample. Each of the models produces a ranking of the 5000 predictors according to their importance with respect to discriminating between  the tumor types, and the agreement of these rankings across the different sub-samples are used to assess the stability of ranking the importance of the predictors. For the Random Forest classifier the predictors are ranked according to the Gini index, while for the logistic Lasso and Ridge regression models we order the predictors by absolute magnitude of the estimated regression coefficients. For the PLS-DA model the importance of the predictors is based on a weighted sum of the absolute coefficients where the weights are proportional to the reduction in the sums of squares across the components. To assess the stability of the individual risk predictions we apply the predictors from the DACOVA data set to each of the models. These predicted probabilities are then ranked in decreasing order, and the agreement of the risk predictions are similarly obtained by the sequential rank agreement applied across the different sub-samples of the training data. We use 1000 iterations for the sub-sampling procedure. 

All four models depend on tuning parameters. We estimate these by a 20 times repeated 5-fold cross-validation on the training data in each iteration. The implementation of Lasso and Ridge regression in the \texttt{glmnet} package offers three different cross-validated optimization criteria for the penalty parameter: total deviance, classification accuracy and Area Under ROC. We apply all three criteria to our data to investigate their effect on the agreement.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=.5\textwidth]{pics/riskAgreementPlot}%
\includegraphics[width=.5\textwidth]{pics/predictorAgreementPlot}
\end{center}
 \caption{Sequential rank agreement of predicted risk probabilities for the DACOVA data (left) and predictor importance (right) based on 1000 sub-samples of the MALOVA training data.}
 \label{fig:app1}
\end{figure}

Figure \ref{fig:app1} (left) shows the sequential rank agreement applied to the estimated risk probabilities of the $113$ patients in the DACOVA data set obtained from the $1000$ sub-samples of the MALOVA data, and Figure \ref{fig:app1} (right) shows the sequential rank agreement of the estimated importance of the 5000 predictors. For clarity we only show the agreement for the latter up to list depth 600 where all the curves have approximately become stable. We note that the predictor importances estimated with the Lasso model are capable of producing censored lists depending on the value of the penalty parameter. Under the deviance optimization criterion the median number of non-zero coefficients was 33 (range 18 to 52). In order to calculate the sequential rank agreement we therefore applied Algorithm \ref{sra-algorithm} with $B = 1000$ and $P=5000$. The effect of the censoring caused by the sparsity of the Lasso is clearly visible in the agreement curves.

For most of the sequential rank agreement curves we see, as expected, that they start low followed by an increase until they approximately become constant.  This has the interpretation that the classification methods show higher agreement with respect to the highest ranked items (the most high-risk patients and the most important predictors respectively) than for the items deeper in the lists across the different sub-samples. The change-points where the curves become approximately constant are the list depths where the ranks of the remaining items become close to random. 

The only exceptions to the expected shape of the agreement curves are the predictor importance agreements from the Ridge models and the risk prediction agreement from the Lasso model with the AUC criterion. They all show higher disagreement in the top of the lists followed by a decrease. The explanations for their behavior is different for the two methods. For the Lasso model the reason is that optimizing the penalty parameter with respect to the AUC criterion tends to favor a very high penalty value causing only a single predictor to be selected in each of the 1000 iterations. This results in a lack of generalizability to the DACOVA data which gives rise to the higher disagreement in the predicted risk probabilities. In the extreme case where the penalty becomes so high that none of the predictors are selected by the Lasso, the sequential rank agreement for the predicted probabilities becomes undefined since all the ranks will be ties. 

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=.5\textwidth]{pics/ridgeCensoring}%
\includegraphics[width=.5\textwidth]{pics/aucPlot}
\end{center}
 \caption{Sequential rank agreement for Ridge regression with an artificial censoring on the absolute value of the regression coefficients (left) and box plots of AUC values across the $1000$ sub-samples with respect to the known class labels of the DACOVA data (right).}
 \label{fig:app2}
\end{figure}

The reason behind the behavior of the sequential rank agreement of predictor importances estimated by the Ridge models is more subtle. Looking at the distribution of the regression coefficients it is seen that a large proportion of them are numerically very close to zero and of almost equal absolute value. This implies that when they are ranked according to their magnitude, their actual order becomes more uncertain and more close to a random permutation. This problem can be alleviate by introducing an artificial censoring depending on the absolute value of the coefficients. Figure \ref{fig:app2} (left) shows the sequential rank agreement curve of the rankings of the predictor importances from the Ridge model using the deviance criterion (same curve as in figure \ref{fig:app1} (left)) as well as an agreement curve where the absolute value of the regression coefficients have been censored at the $0.1\%$ quantile. In our application the median number of censored predictors across the 1000 sub-samples is 5 (range 1 to 11). Even though the number of censored predictors is very small compared to the total number of predictors, the effect on the sequential rank agreement is substantial as it has now flipped to show higher agreement in the top of the lists as was expected. 

Returning to Figure \ref{fig:app1}, it can further be seen that some of the methods show better agreement with respect to the predicted probabilities than for ranking the importance of the predictors and vice versa. Ridge regression performs very well for the risk predictions, but PLS-DA performs better for the predictor importances and similarly worse for the risk predictions. The Random Forest method seems to perform equally well in both scenarios. Lasso regression (except with the AUC criterion) performs very well at ranking the high-risk patients but very poorly at ranking the importance of the predictors. This reason for this is the high auto-correlation between the intensities in the mass spectra which leads to collinearity issues in the regression models. It is well-known that variable selection with the Lasso penalty does not preform very well when the predictors are highly correlated. This collinearity does, however, not affect the agreement of the risk predictions, as seen on the left part of the figure, since it is not so important which specific variable that gets selected from a group of highly correlated predictors when the purpose is risk predictions.


Finally, it would appear that Ridge regression with the AUC criterion for the penalty parameter achieves the best performance with respect to the stability of ranking the individual predicted risk probabilities. It must, however, be stressed that the sequential rank agreement in this application is only concerned with whether similar orderings of the patients is obtained under perturbations of the training data. It is not concerned with whether such an ordering is the correct one. This is similar to the usual distinction between accuracy and precision. Looking at the distributions of the AUCs illustrated in Figure \ref{fig:app2} (left) across the $1000$ sub-samples for a selection of the models with respect to the known class labels of the DACOVA data, we see that PLS-DA attains the highest AUC with a median value of $0.71$ while the Ridge model with the AUC criterion attains a median AUC of $0.49$. This implies that while Ridge regression optimized with respect to the AUC criterion achieves the best sequential rank agreement it performs similar to a random coin toss with respect to classifying the DACOVA patients. Both concerns are of course important in practice and for this data set it appears that PLS-DA is the overall best method taking both agreement and classification accuracy into account.
...
lists. In addition, censored ranked lists where only the top $k$
ranked items are known can be accommodated as well. The proposed
agreement measure can be interpreted as the average distance between
an item's rank and the average rank assigned to that item and thus
determine how well the lists agree on the ranks of a specific
item and it circumvents the problem with other existing similarity measures.
...
method by bootstrapping the data repeatedly and comparing the ranks
obtained from the bootstrapped data.

Our applications show that XXX
Through simulations we showed that XXX --- afventer Andreas.
...
Finally, we have --- whenever possible --- used as much information
from the lists as possible, i.e., we have used all available ranks.
We could choose to censor the rankings of all the items that were
deemed not to be significant if information on both rankings and the
corresponding evidence for significance was available. That would
...
agreement among the items that were deemed to relevant.
...
the two predictors high while the other might have a very low
rank. When the rankings from these two analysis methods are compared
then the agreement will differ simply because of the way correlated
...
applied to a large number of situations.  The method can be adapted
even further by using it to identify and cluster statistical analysis
methods that provide more or less the same agreement, or by using the
rank agreement to optimize a hyper-parameter in, say, elastic net regularized
regression where the rank agreement is used to determine the mixing
proportion between the $L_1$ and the $L_2$ penalty. Finally, the
proposed method may be adapted (with some additional assumptions) to
the situation where the lists are subsets of each other. We will be
investigating these extensions further in the future.
