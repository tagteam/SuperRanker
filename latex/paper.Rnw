...
predictors when we have censored lists because we only do not know the
exact rank assigned to each item in all lists.  Instead we assume that
the rank assigned to an item $X_p$ that is not present in list $l$
(i.e., is censored) is uniformly distributed among the ranks that have
\emph{not} been assigned for list $l$.  The rankings within a single
list are clearly not independent since each rank must appear exactly
in each list. Thus we cannot simply assign



Thus, by randomizing unobserved items as described in
\eqref{formula:censored-agreement}
...
xvals <- 1:500
plot(mmyres, xlim=c(1, 300), type="l")
