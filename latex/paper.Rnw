...
predictions. Thus, we also computed the AUC and Brier scores for the different models
...
\textbf{Comment on Brier scores}
Both concerns are of course important in practice and for this data set 
it appears thatPLS-DA is the overall best method taking both agreement 
and classification accuracy into account.
