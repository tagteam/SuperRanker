...
task. As Hall and Schimek \mcomment{REF}, we work with an infinite set
...
\begin{equation}
  d^\ast(q) = \min\left\{\argmin_{d\in\{1,\dots,P\}}\textrm{sra}(d)>q, P\right\}.
\end{equation}
...
\begin{equation}
\widehat{d^\ast_L}(q) = \min\left\{\argmin_{d\in\{1,\dots,P\}}\widehat{\textrm{sra}}_L(d)>q, P \right\}
\end{equation}
...
(\ref{eq:sumset}) is equal to the counting process evaluated at the last element:
\begin{equation*}
  |S(d)| = N(P;d).
\end{equation*}
...
uniformly in $d$ and because $A(X_p)\le 2P^2$ u uniformly in $p$ \mcomment{double check bound}. It
...
\end{align}
It follows directly from the law of large numbers that
...
finite second moment since $A(X_p)\le 2L^2$). Again by equation
(\ref{eq:countingConv}) it follows that
$\widehat{N}_L(P; d) = N(P;d) + o_P(1)$ uniformly in $d$ and therefore
by the continuous mapping theorem and since $N(P;d)>0$) the first factor
