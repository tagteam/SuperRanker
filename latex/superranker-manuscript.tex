...
uniform distribution which assigns probability $(1/P)^P$ to each of
the $P^P$ different rankings.
...
can be measured by the expected variance across the lists:
\begin{equation}
  {A}(X_p) = \sum_{r=1}^{P^P} \{r(X_P)-\E_Q R(X_P)\}^2 Q(r)
\end{equation}
where $\E_Q R(X_p)=\sum_{r=1}^{P^P} r(X_p)Q(r)$. The empirical
counterpart is
...
\bigskip\hrule HOV
Vi kunne sige her: Alternative distance functions (f non-negativ real valued) are discussed in section ??
\hrule\bigskip 
% \begin{equation}
  % \widehat{A_L}(X_p) = f(R_1(X_p), \ldots, R_L(X_p))
% \end{equation}
...
lists:
\begin{equation}
S_d = \{X_p; \left(\sum_{r=1}^{P^P} I\{r(X_p)\le d\} Q(r)\right) >0\}.
\end{equation}
The empirical counterpart the set of unique items ranked less than or
equal to $d$ in any of the $L$ lists:
\begin{equation}
\widehat{S}_{d,L} = \{X_p; \left(\sum_{l=1}^L I\{R_l(X_p)\le d\}\right)>0\},
\end{equation}
...
expected agreement of the items found in the set $S_d$:
\begin{equation}
  \textrm{sra}(d) = \frac{\sum_{\{p \in S_d\}}A(X_p)}{|S_d|}\label{def:sra}
  % & =\sum_{p=1}^P \sum_{l=1}^L I\{R_l(X_p)\le d\}>0\}\notag
\end{equation}
 where $|S_d|$ is the cardinality of the set $S_d$.
The empirical counterpart is given by
\begin{equation}
  \widehat{\textrm{sra}}_L(d) = \frac{\sum_{\{p \in \widehat{S}_{d,L}\}}(L-1)\widehat{A_L}(X_p)}{(L-1)|\widehat {S}_{d,L}|}.\label{def:sra}
                                % \intertext{where}
                                % |S_d| & =\sum_{p=1}^P \sum_{l=1}^L I\{R_l(X_p)\le d\}>0\}\notag
\end{equation}
% where is the cardinality of the set $\widehat{S}_{d,L}$.
...
The sequential rank agreement then becomes equivalent to the pooled
variance of the items found in $S_d$, and it can be interpreted as the
...
\subsection{Population parameter}

Given a sample of $1,\ldots,L$ lists an estimator of the sequential
rank agreement is given by
\begin{equation}
