...
\begin{abstract}
  ...
  any number of lists even if some or all are incomplete. The approach
  can identify changepoints in the agreement of the lists, and the
  sequential changes of agreement as a function of the depth of the
  lists can be compared graphically to a permutation based reference
  set or evaluated using analytical asymptotic results.  The
  usefulness of these tools are illustrated using gene rankings, and
  using data from two Danish ovarian cancer studies where we assess
  the within and between agreement of different statistical
...
rank agreement will be zero for all position or depths $d$ down the
lists.  The sequential rank agreement is equivalent to the pooled
variance of the items found in $S(d)$, and it can be interpreted as
...
For an easier interpretation we suggest to plot
$\sqrt{\widehat{\textrm{sra}}_L(d)}$ corresponding to the pooled
standard deviation.
...
The sequential rank agreement curve shown in the two left plots of
Figure~\ref{fig:example1} roughly indicates the average distance in
ranks for the genes found among the first $d$ positions in the $4$
lists.  It can be seen that the sequential rank agreement limits are
better towards the top of the lists (smaller values on the $y$ axis
...
there is a substantial drop in agreement (higher sra) even after the
first depth. Thus, if we were to restrict attention to a small set of
predictors then we would focus on predictors 2124, 829, and 378 as
seen in Table~\ref{tab1}.
...
\begin{table}[tb]
\centering
\caption{List of ranked results from the Golub data. Numbers indicate the predictor/gene for the given ranking and method. Only the top 10 ranks are shown in the table.} 
\label{tab1}
\begin{tabular}{rrrrr}
  \hline
...
\begin{figure}[htbp]
 ...
 \caption{Left panels: Sequential rank agreement for 4 different
   ...
   line). Right panels: Corresponding sequential rank agreement for
   the same data but where only the top 20 ranked items are available
   and the rank of the remaining items are not available. The blue and
...
partial ordering.  Incomplete lists also occur if the analyst
...
ranks that are missing from the lists provide so little information that it is reasonable
...
to the depths where the number of items without ranks available is
...
become incomplete. This is an artefact stemming from the fact that we assume
that the remainder of the lists can be replaced by a simple
permutation of the missing items.

The two right-most plots of Figure~\ref{fig:example1} show the impact
of restricting the Golub data such that only top-20 lists are
available instead of full lists of length 1000 (20 was chosen to
resemble the list lengths that might be published in a
manuscript). The sequential rank agreement increases much quicker
...
the methods used to generate the individuals rankings in the $L$ lists
by only requiring that there is no information contained in the ranking but that
the rankings are all based on applying the method/approaches to the
same data
\begin{eqnarray*}
...
for list $l$ from  $d_l$ to $P$ (\ie, each list so it has the same number of
...
no information since it lies below the reference areas. The incomplete
...
depth agreement. For the sequential rank agreement we use the function
\texttt{sra} of the \texttt{R} package \texttt{SuperRanker} with the
\texttt{"mad"} option (median absolute distance). To define the depth
of agreement we set a constant threshold function to an integer $q$
and report the first crossing point, i.e., the smallest list depth
where sra exceeds $q$. For topK we use the function
...
\texttt{maxK} as the depth of agreement. Thus, to make this comparison
we assume that the first crossing point of sra and the result of the
topK method measure the same underlying feature.

In each simulation run, we generated a dataset with 1000 predictors
and 400 observations. The predictors were drawn independently from a
standard Gaussian distribution. The outcome was drawn from a Gaussian
distribution with variance 1 and mean that depended additively on the
first 15 predictors, i.e.,
...
predictors according to their marginal $t$ test statistics. Thus, we
assessed the depth of agreement among lists that are ranked with the
same statistical method on bootstrap versions of the same dataset. We
report simulation results from two scenarios where each is based on
1000 simulated datasets:
\begin{enumerate}
\item[] Scenario I: Fix \(L=8\) and vary $q\in\{3,4,5,6,7,8,9,10\}$.
\item[] Scenario II: Fix \(q=5\) and vary $L\in\{3,5,10,50\}$.
\end{enumerate}
In both scenarios we summarized the distribution of the estimated
depth of agreement and the average number of unique predictors found
...
methods. We see that sra results in a substantially lower depth of
agreement than the topK method. Also the average numbers of unique
predictors (bold numbers inside the plots) are markedly smaller for
sra. Even larger differences were found when we used the Euclidean
distance instead of the median absolute distance for the sequential
rank agreement (results not shown). As expected the depth of agreement
increased when the threshold for agreement/window increased.
\begin{figure}[htbp]
   \begin{center}
...
Scenario II. The number of lists has little impact on the
results. Generally, sra is more conservative than topK and as a
consequence sra includes fewer predictors in the selected set where
the lists agree.
