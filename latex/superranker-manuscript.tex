...
\begin{equation}
  {A}(X_p) = \sum_{r=1}^{P!} \{r(X_P)-\E_Q R(X_P)\}^2 Q(r)
\end{equation}
...
\begin{equation}
  \widehat{\textrm{sra}}_L(d) = \frac{\sum_{\{p \in \widehat{S}_{L}(d)\}}(L-1)\widehat{A_L}(X_p)}{(L-1)|\widehat {S}_{L}(d)|}.\label{def:sraEst}
\end{equation}
...
\item{$\E_Q[R(X_p)^4]$ is finite for all $p \in 1,\ldots,P$}.
\item{$|S(d)| > 0$ uniformly in $d$}.
\end{enumerate}
\end{assumption}
We note that assumption $B$ implies that for every list depth $d$ there must be a non-zero probability of ... under the probability measure $Q$.
...
\end{align}
which jumps at a time $p$ if the probability that the rank of $X_p$ is less than or equal to $d$ is bounded away from zero under the probability measure $Q$.
...
\end{align}
Since the joint distribution of $\left\{\widehat{N}_L(p;d) : p=1,\ldots,P\right\}$ is completely determined by its finite set of jump times and $\widehat{Q}_L(R(X_p) \leq d) \overset{P}{\longrightarrow} Q(R(X_p) \leq d)$ for every $p$ and $d$ for $L \rightarrow \infty$ by the law of large numbers, it follows that
\begin{align}
  \sup_{d \in 1,\ldots,P}\sup_{p \in 1,\ldots, P} \left|\widehat{N}_L(p; d) - N(p; d)\right| = o_P(1)\label{eq:countingConv}
\end{align}

Recalling the definition of the sequential rank agreement in equation (\ref{def:sra}), it may be rewritten as the following integral with respect to the counting measure induced by $N$
\begin{align}
  \textrm{sra}(d) = \int_{1}^P \frac{A(X_p)}{N(P; d)}dN(p; d)
\end{align}
where we note that the cardinality of the set $S(d)$ in equation (\ref{eq:sumset}) is equal to the counting process evaluated at $P$.
The empirical sequential rank agreement is similarly given by
\begin{align}
  \widehat{\textrm{sra}}_L(d) &= \int_1^P \frac{\widehat{A_L}(X_p)}{\widehat{N}_L(P; d)}d\widehat{N}_L(p; d) 
\end{align}
...
\begin{align}
  ...
  V_L(d) &=  \int_1^P \left(\frac{\widehat{A_L}(X_p)}{\widehat{N}_L(P; d)} - \frac{A(X_p)}{N(P;d)}\right)d\widehat{N}_L(p; d)
\end{align}
The conclusion of the proof therefore follows if each of these two terms are uniformly of order $o_P(1)$ in $d$.

Starting with $V_L(d)$, since $\widehat{N}_L$ is of bounded variation it follows that
\begin{align}
 V_L(d) &\leq \sup_{p \in 1,\ldots,P}\left|\frac{\widehat{A_L}(X_p)}{\widehat{N}_L(P; d)} - \frac{A(X_p)}{N(P;d)}\right| \int_1^P \left|d\widehat{N}_L(p; d)\right|\label{eq:secondTerm}
\end{align}
Looking at $\widehat{A_L}(X_p)$ we have that
\begin{align}
  ??
\end{align}
Therefore, under assumption A) we have that ... and therefore $\widehat{A_L}(X_p) = A(X_p) + o_P(1)$ uniformly in $p$. A consequence of equation (\ref{eq:countingConv}) is that $\widehat{N}_L(P; d) = N(P;d) + o_P(1)$ uniformly in $d$ and therefore by the continuous mapping theorem and assumption B) the first factor in equation (\ref{eq:secondTerm}) is of order $o_P(1)$ uniformly in $p$ and $d$. The result then follows by noting that the second factor is bounded by $P$ uniformly in $d$. 

Continuing with $U_l(d)$ we have that
\begin{align}
	...
\end{align}
...
\begin{align}
S_d &= \left\{X_p; \left(\sum_{r=1}^{P!} I\{r(X_p)\le d\} Q(r)\right) >0\right\}\\
    &= \left\{X_p; \E[I\{R(X_p)\le d\}] >0\right\}\\
    &= \left\{X_p; P(R(X_p)\le d) > 0\right\}\\
\widehat{S}_{d,L} &= \left\{X_p; \left(\sum_{l=1}^L I\{R_l(X_p)\le d\}\right)>0\right\}
\end{align}
