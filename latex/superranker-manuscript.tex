...
S(d) &= \left\{X_p; \left(\sum_{r=1}^{P!} 1\left(r(X_p)\le d\right) Q(r)\right) >0\right\}\label{eq:sumset}\\
     &= \left\{X_p; Q(R(X_p) \leq d) > 0 \right\}\nn
\end{align}
where $1(\cdot)$ denotes the indicator function.
The empirical counterpart the set of unique items ranked less than or
...
task. As Hall and Schimek \mcomment{REF}, we work with an infinite set of lists
and study the asymptotic behaviour for $L\to\infty$.
However, we do not allow the length of the lists to
change with $L$ as in most applications the length of the lists is fixed.

We start by showing that $\widehat{\textrm{sra}}_L$ is a consistent estimator of $\textrm{sra}$ for $L \rightarrow \infty$.
The proof is given in Theorem \ref{thm:consistency} below and requires the following assumptions.
\begin{assumption}
We assume that:
\begin{enumerate}[label=\Alph*)]
\item{The $L$ lists are independent and identically distributed samples from $Q$.}
\item{$\E_Q[R(X_p)^2] < \infty$ uniformly in $p$.}
\item{$|S(d)| > 0$ uniformly in $d$}.
\end{enumerate}
\end{assumption}
We note that assumption $C$ implies that for every list depth $d$ there must be a non-zero probability of ... under the probability measure $Q$.

\begin{theorem}
Under the assumptions it follows that $\left\|\widehat{\textrm{sra}}_L - \textrm{sra}\right\|_\infty = o_P(1)$.
\label{thm:consistency}
\end{theorem}


We now define the parameter of interest. For any given threshold value $q$, 
a change-point $d^\ast(q)$ in the list agreement is defined by
\begin{equation}
  d^\ast(q) = \argmin_{d\in\{1,\dots,P\}}\textrm{sra}(d)>q.
\end{equation}
The value of $q$ could for example be the agreement of randomly
permuted lists. The corresponding estimated change-point is similarly defined by
\begin{equation}
\widehat{d^\ast_L}(q) = \argmin_{d\in\{1,\dots,P\}}\widehat{\textrm{sra}}_L(d)>q
\end{equation}
The consistency of $\widehat{d^\ast_L}(q)$ then follows by a simple corollary. 
...
\mcomment{Comment on how to apply this in practice (finite sample/bootstrap/etc.) and that $q$ is similar to LoA BA-style.}
...
\end{align}
which jumps with an increment of one at time $p$ if the probability that the rank of $X_p$ is less than or equal to $d$ is bounded away from zero under the probability measure $Q$.
Similarly, we have the empirical counterpart
\begin{align}
...
\end{align}
The joint law of $\{\widehat{N}_L(p;d)\}_{1\leq p \leq P}$ is completely determined by its finite set of jump times, and because
$1(R_l(X_s) \leq d)$ are independent and identically Bernoulli distributed over $l$ with expectation $Q(R(X_s) \leq d)$ 
it follows from the law of large numbers that $\widehat{Q}_L(R(X_p) \leq d) \overset{P}{\longrightarrow} Q(R(X_p) \leq d)$ for every $p$ and $d$ 
for $L \rightarrow \infty$, and therefore
\begin{align}
...
The sequential rank agreement given in equation (\ref{def:sra}) may therefore be rewritten as the following integral with respect to the counting measure induced by $N$
\begin{align}
  \textrm{sra}(d) = \int_{1}^P \frac{A(X_p)}{N(P; d)}dN(p; d)
\end{align}
where we note that the cardinality of the set $S(d)$ in equation (\ref{eq:sumset}) is equal to the counting process evaluated at time $P$.
The empirical sequential rank agreement is similarly given by
\begin{align}
  \widehat{\textrm{sra}}_L(d) &= \int_1^P \frac{\widehat{A}_L(X_p)}{\widehat{N}_L(P; d)}d\widehat{N}_L(p; d) 
\end{align}
and by elementary algebra it follows that
\begin{align}
...
where
\begin{align}
  U_L(d) &= \int_1^P\frac{A(X_p)}{N(P;d)} d\left(\widehat{N}_L(p; d) - N(p; d)\right)\\
  V_L(d) &=  \int_1^P \left(\frac{\widehat{A}_L(X_p)}{\widehat{N}_L(P; d)} - \frac{A(X_p)}{N(P;d)}\right)d\widehat{N}_L(p; d)
\end{align}
The conclusion of the proof therefore follows if each of these two terms are uniformly of order $o_P(1)$ in $d$.

Starting with the first term, we have that
\begin{align}
...
\end{align}
where the second factor is $o_P(1)$ uniformly in $d$ by equation (\ref{eq:countingConv}). The first factor is $O(1)$ since $N(P;d)$ uniformly bounded away from zero in d by assumption C and $A(X_p)$ is uniformly bounded in $p$ by assumption B. It thus follows that $\left|U_L(d)\right| = o_P(1)$ uniformly in $d$.

Continuing with $V_L(d)$ we may similarly bound as
\begin{align}
 V_L(d) &\leq \sup_{p \in 1,\ldots,P}\left|\frac{\widehat{A}_L(X_p)}{\widehat{N}_L(P; d)} - \frac{A(X_p)}{N(P;d)}\right| \int_1^P \left|d\widehat{N}_L(p; d)\right|\label{eq:secondTerm}
\end{align}
It follows directly from the law of large numbers that $\widehat{A_L}(X_p) = A(X_p) + o_P(1)$ uniformly in $p$ since $R_i(X_p)$ in equation (\ref{eq:empDistance}) for $i=1,\ldots,L$ are independent and identically distributed random variables from $Q$ with finite second moment by assumption B). Again by equation (\ref{eq:countingConv}) it follows that $\widehat{N}_L(P; d) = N(P;d) + o_P(1)$ uniformly in $d$ and therefore by the continuous mapping theorem and assumption C) the first factor in equation (\ref{eq:secondTerm}) is of order $o_P(1)$ uniformly in $p$ and $d$. The result then follows by noting that the second factor is bounded by $P$ uniformly in $d$. 
